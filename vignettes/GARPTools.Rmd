...
We also provide example environmental layers (altitude [alt], annual mean tmeperature [bio_1], annual precipitation [bio_12]) to use in GARP, available as a raster stack (env_layers.TIF) or individually as ASCI grid files:
```{r comment = "R>", echo = -1, message=FALSE}
library(raster)
...
par(mar=c(2,2,2,1))
aucRatio(n = 10, x = r.sum, points = test.pts, E = 0.20)
