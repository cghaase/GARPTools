---
title: 'GARPTools: An R Package for GARP Data Preparation and Model Assessment'
...
We also provide example environmental layers (altitude [alt], annual mean tmeperature [bio_1], annual precipitation [bio_12]) to use in GARP, available as a raster stack (env_layers.TIF) or individually as ASCI grid files in the "extdata" folder:
```{r comment = "R>", message=FALSE}
library(raster)
...
Plotting the rasters allows for the visualization of all environmental layers (Figure 1).
```{r fig.height=2, fig.align = "center", fig.cap = "Three example environmental layers used in GARP experimental runs."}
par(mfrow = c(1,3))
plot(alt,    main = "Altitude (m)")
plot(bio_1,  main = "Annual Mean Temperature (C)")
plot(bio_12, main = "Annual Precipitation (mm)")
```

Now add the boundary of the sampling area and sampling points to the altitude layer (Figure 2).
```{r comment = "R>", echo = -1, fig.width=4, fig.height=3, fig.align = "center", fig.cap = "Altitude (m) of sampling area with sampling points."}
par(mar=c(3,2,2,1))
data("nc_boundary")
plot(alt)
...
Here, we provide the rasters into a "raster" folder and run the resample function to save the resampled rasters into a new folder. Note that a Warning will be provided if the projection of any raster is missing:
```{r comment = "R>", message=FALSE, warning=FALSE, results = "hide"}
...
We have provided an example output from GARP in a "tasks" folder. We can sum the best subset model outputs into a single summed raster using the `sumRasters` function:
```{r comment = "R>"}
...
AUC is calculated by overlaying one of the testing points (`points` argument) withheld from the sample data using the `splitData` function on top of the summed raster from the `sumRasters` function (`x` argument). Here we plot an example test shapefile, "wtdeer_test1.shp":
...
par(mar=c(2,2,2,1))
aucGARP(n = 10, x = r.sum, points = wtdeer1_test)
```
...
However, AUC can often undervalue certain niche models, such as those provided by GARP, and lead to incorrect comparisons among model variations. Thus, following methods outlined by Peterson et al [(2008)](#anchor3), we also provide a means to calculate partial-area ROC and AUC ratios that provide a means to better compare these models.

`AUCratio` allows the user to define the error parameter, `E`, which refers to the amount of error admissible along the true-positive (sensitivity) axis, i.e., how much omission error is accetable. When highest-quality data are used, `E`= 0, and when occurrence data are known to include high amounts of error, `E` can range from 5-20%:
```{r echo = -1, comment = "R>", fig.width=4, fig.height=4, fig.align = "center", fig.cap = "Plot of the Modified Receiver Operating Characteristic (ROC) curve."}
par(mar=c(2,2,2,1))
aucRatio(n = 10, x = r.sum, points = wtdeer1_test, E = 0.20)
...
```{r comment = "R>"}
commissionGARP(n = 10, x = r.sum, points = wtdeer1_test, 
               file.path = system.file("extdata/BestSubsets/", package="GARPTools"))
omissionGARP(n = 10, x = r.sum, points = wtdeer1_test)
```

Now that we have the AUC score, the AUC ratio, the commission and omission metrics, and the z-scores and standard error, we can produce a table that ranks the 10 models from highest to lowest AUC scores:


Model |  AUC   |    AUC Ratio     |    Total Commission | Average Commission  | Total Omission | Average Omission
------|:------:|:----------------:|:-------------------:|:-------------------:|:--------------:|:-----------------:
8     | 0.72   |     1.28         |       18.47         |    53.80            |   0.00         |  18.15
9     | 0.72   |     1.28         |       24.99         |    51.01            |   0.11         |  19.22
4     | 0.72   |     1.28         |       26.97         |    61.88            |   0.00         |  13.33
1     | 0.69   |     1.25         |       28.48         |    51.82            |   0.19         |  22.89
10    | 0.68   |     1.24         |       35.63         |    59.63            |   0.00         |  18.89
3     | 0.68   |     1.24         |       26.50         |    54.27            |   0.15         |  25.50
5     | 0.66   |     1.22         |       31.46         |    56.98            |   0.07         |  26.27
7     | 0.66   |     1.22         |       18.41         |    51.22            |   0.07         |  28.12
6     | 0.62   |     1.16         |       27.13         |    51.06            |   0.04         |  35.17
2     | 0.61   |     1.14         |       37.04         |    52.75            |   0.19         |  32.15
...
Dominant rules for each model are defined as the subset of rules that together predict over 90% of the landscapes. The `findModRules` function is used to extract a table of dominant rules for the best subsets.

First define a path (`pathA`) which contains the directories of for ruleset grids produced by GARP (e.g., folders containing the term "grid") and a path (`pathB`) that contains the directories for the best subsets for each species (e.g. folders containing the term "task"), then run the function:
...
```{r comment = "R>"}
write.csv(rangelogit, paste(system.file(package = "GARPTools"),
                            "/extdata/RangeLogit.csv", sep=""))
...
prevalence <- prevalence(pathA = paste(system.file(package = "GARPTools"), 
                                       "/extdata/RangeLogit.csv", sep=""), 
                         pathB = system.file("extdata/resampled/", 
                                             package="GARPTools"),
                         num = 10)
```
...
```{r comment = "R>"}
write.csv(prevalence, paste(system.file(package = "GARPTools"), 
                            "/extdata/Prevalence.csv", sep=""))
```
...
```{r comment = "R>"}
write.csv(UnimportIndex, paste(system.file(package = "GARPTools"), 
                               "/extdata/UnimportanceIndex.csv", sep=""))
```
...
Peterson AT, Papes M, Soberon J [(2007)]{#anchor3} Rethinking receiver operating characteristic analysis applications in ecological niche modeling. Ecological Modelling 213(1):63-72.
